...
async function demonstrateDataModels() {
  ...
  console.log('[ok] Storage service initialized\n');
  ...
  console.log(`[ok] Created presentation: ${presentation.title}`);
  ...
  console.log('[ok] Added clients to session');
  ...
  console.log(`[ok] Navigated to slide ${session.getCurrentSlide()}\n`);

  // Update laser pointer
  session.updateLaserPointer(150, 200, true);
  const laserPointer = session.getLaserPointer();
  console.log(`[ok] Laser pointer activated at (${laserPointer.x}, ${laserPointer.y})\n`);
  ...
  console.log(`[ok] Added drawing stroke to slide ${slide.order + 1}`);
  ...
  await storage.savePresentation(presentation);
  console.log('[ok] Updated session and presentation in storage\n');

  // Get storage statistics
  const stats = await storage.getStorageStats();
  console.log('[stats] Storage Statistics:');
  ...
  console.log('[ok] Validation Results:');
  ...
  console.log('[json] JSON Serialization:');
