...
const { findDemoC } = require('./polyglotDeckCapture');
...
/**
 * Variables expected by hacksu/polyglot Jinja templates (see presentation/app.py).
 * Extend here if you add new template variables.
 */
async function loadPolyglotTemplateContext(presentationDir) {
  const demoPath =
    (await findDemoC(presentationDir)) || (await findDemoC(path.dirname(presentationDir)));
  if (!demoPath) {
    throw new Error(
      'HacKSU presentation format needs examples/demo.c. Zip the repo so examples/ sits next to presentation/ (same layout as hacksu/polyglot).'
    );
  }
 ...
 */
async function renderPolyglotIndexHtml(presentationDir) {
  ...
  if (!(await fileExists(indexTpl))) {
    throw new Error('Expected presentation/templates/index.html for this HacKSU deck.');
  }
  const ctx = await loadPolyglotTemplateContext(presentationDir);
...
module.exports = {
  renderPolyglotIndexHtml,
  loadPolyglotTemplateContext
};
